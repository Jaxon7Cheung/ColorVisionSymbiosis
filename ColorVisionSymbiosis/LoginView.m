...
- (void)setUI {
    self.Screen_WIDTH = self.bounds.size.width;
    self.Screen_HEIGHT = self.bounds.size.height;
    self.MINIGAP = self.bounds.size.width / 16;
    //漂浮球
    self.backgroundColor = [UIColor colorWithRed: 33 / 255.0 green: 33 / 255.0 blue: 33 / 255.0 alpha: 1.0];
    self.circleView = [[UIView alloc] initWithFrame: CGRectMake(0, _MINIGAP, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height / 3.5)];
    ...
    self.whiteView.layer.cornerRadius = _Screen_WIDTH / 8;
    [self addSubview: self.whiteView];
    [self.whiteView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(self).multipliedBy(2);
        make.top.mas_equalTo(self.circleView.mas_bottom).mas_offset(_MINIGAP / 1.5);
    }];
    ...
    [self.titleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(_Screen_WIDTH / 1.75, _MINIGAP * 2.25));
        make.top.mas_equalTo(self.whiteView).mas_offset(_MINIGAP * 1.25);
    ...
    [self.phoneField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(_Screen_WIDTH * 0.76, _MINIGAP * 2.2));
        make.top.mas_equalTo(self.titleView.mas_bottom).mas_offset(_MINIGAP * 2.5);
        make.centerX.mas_equalTo(self);
    }];
...
//    self.phoneField.contextMenuInteractionEnabled = NO;
    ...
    [self.passwordField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(_Screen_WIDTH * 0.76, _MINIGAP * 2.2));
        make.top.mas_equalTo(self.phoneField.mas_bottom).mas_offset(_MINIGAP * 2);
...
//    self.passwordField.autocorrectionType = UITextAutocorrectionTypeNo;
    ...
    [self.loginButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(_Screen_WIDTH * 0.76, _MINIGAP * 2.2));
        make.top.mas_equalTo(self.passwordField.mas_bottom).mas_offset(_MINIGAP * 1.5);
        make.centerX.mas_equalTo(self);
    }];
    ...
    [self.phoneLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(self.phoneField.mas_top).mas_offset(-_MINIGAP / 3);
        make.left.mas_equalTo(self.phoneField);
    }];
    ...
    [self.passwordLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.mas_equalTo(self.passwordField.mas_top).mas_offset(-_MINIGAP / 3);
        make.left.mas_equalTo(self.phoneField);
    }];
    ...
    [self.loginByCodeButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.loginButton.mas_bottom).mas_offset(_MINIGAP / 3);
...
- (void)WayOfLoginingByCode {
    ...
    [self.passwordField mas_updateConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(_Screen_WIDTH * 0.46, _MINIGAP * 2.2));
    }];
    
    //验证码按钮
    self.getCodeButton.hidden = NO;
    [self.getCodeButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.passwordField.mas_right).mas_offset(_MINIGAP / 3);
...
- (void)presentWrongMessage: (NSString *)messageText {
    if (!self.wrongMessageLabel) {
        self.wrongMessageLabel = [[UILabel alloc] init];
//        self.wrongMessageLabel.text = @"请输入手机号或密码";
        self.wrongMessageLabel.text = messageText;
        self.wrongMessageLabel.textColor = [UIColor redColor];
        self.wrongMessageLabel.font = [UIFont systemFontOfSize: 12];
        [self addSubview: self.wrongMessageLabel];
        [self.wrongMessageLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.width.mas_equalTo(self.phoneField.mas_width);
                make.left.mas_equalTo(self.phoneField.mas_left);
                make.top.mas_equalTo(self.passwordField.mas_bottom).mas_offset(9.5);
        }];
    }
}

- (void)removeWrongMessage {
    [self.wrongMessageLabel removeFromSuperview];
    self.wrongMessageLabel = nil;
}
